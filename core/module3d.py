...
class Object3D(object):
    ...
    def PrintInfor(self):
        ...
        print "-----crazylog-------self.fvert = ",len(self.fvert)
        if len(self.fvert) > 0:
            print "-----crazylog-------self.fvert = ", self.fvert[0].shape

        print "-----crazylog-------self.fnorm = ", len(self.fnorm)
        if len(self.fnorm) > 0:
            print "-----crazylog-------self.fnorm = ", self.fnorm[0].shape

        print "-----crazylog-------self.fuvs = ", len(self.fuvs)
        if len(self.fuvs) > 0:
    ...
    def SaveObj(self):
        ...
        with open(path, 'w') as test_txt:
            ...
            for r in range(0,len(self.coord)):
                if len(self.color)>0:
                    #With RGBA
                    line = 'v {} {} {} {} {} {} {}\n'.format(self.coord[r][0],self.coord[r][1],self.coord[r][2],
                                                             self.color[r][0],self.color[r][1],self.color[r][2],self.color[r][3])
                    test_txt.write(line)
            if len(self.vnorm)>0:
                for r in range(0,len(self.vnorm)):
                        line = line +'vn {} {} {}\n'.format(self.vnorm[r][0], self.vnorm[r][1], self.vnorm[r][2])
                        test_txt.write(line)

            for r in range(0,len(self.texco)):
                line = 'vt {} {}\n'.format(self.texco[r][0],self.texco[r][1])
                test_txt.write(line)

            # tri_index_offset = 1
            #     for r in range(0,len(self.texco)):
            #         triangles[r,:] = triangles[r,:] + tri_index_offset
            #         line = 'f {}/{} {}/{} {}/{}\n'.format(triangles[r,0],triangles[r,0],triangles[r,1],triangles[r,1],triangles[r,2],triangles[r,2]) # if with texture coordinate
